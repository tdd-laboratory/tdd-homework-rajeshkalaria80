...
_date_other_pat = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    '''Find tokens that are dates (iso8601)'''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_other(text):
    '''Find tokens that are dates in this format DD MMM YYYY (for example25 Jan 2017)'''
    for match in _date_other_pat.finditer(text):
        yield('date', match)
